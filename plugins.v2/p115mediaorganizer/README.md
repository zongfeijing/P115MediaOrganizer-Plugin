...
- 详情页只展示最近 5 个批次和 5 条明细，可通过按钮打开完整分页历史。
...
插件依赖 `p115client`。如果 MoviePilot 更新或容器重建后依赖丢失，插件会在首次连接 115 时自动执行 `python -m pip install p115client` 并重试导入。如果容器无网络或无安装权限，插件仍会加载，但页面和 API 会显示安装失败原因。
...
- `history_limit` 控制明细保留条数，`run_limit` 控制批次摘要保留数量；完整历史页通过 `/history_page` 分页展示。
