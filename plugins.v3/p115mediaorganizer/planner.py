...
class Planner:
    ...
    def build_plans(
        ...
        for item in items:
            ...
            media_source, media_id = self._media_identity(mediainfo)
    ...
    @staticmethod
    def _media_identity(mediainfo: Any):
        """返回规范的 V3 媒体身份对，拒绝只存在一半的来源或 ID。"""
        if not mediainfo:
            return None, None
        ...
        return (str(source_value).strip(), str(media_id)) if source_value and media_id else (None, None)
    ...
    def _moviepilot_rename_path(media_type: str, mediainfo: Any, meta: Any, ext: str, source_name: str, warnings: List[str]) -> str:
        ...
        try:
            ...
            # V3 通过文件管理模块的公开能力生成推荐名称，避免依赖内部模板构建器。
            rename_path = FileManagerModule.recommend_name(meta=meta, mediainfo=mediainfo) or ""
